  ...
  game_hash.each do |location, team_info|
    team_info.each do |key, value|
      ...
      value.each do |player_hash|
        player_hash.each do |stat_name, stat_data|
          if stat_data == player_name
...
def team_colors(team_name)
  game_hash.each do |location, team_info|
      if team_info[:team_name] == team_name
        return team_info[:colors].collect
      end
    end
end
  ...
  game_hash.each do |location, team_info|
    if team_info[:team_name] == team_name
      ...
      while i < team_info[:players].length
        numbers << (team_info[:players][i][:number])
        i += 1
      end
    end
  end
